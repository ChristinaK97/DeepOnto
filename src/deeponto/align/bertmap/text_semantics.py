...
class AnnotationThesaurus:
    ...
    def hard_nonsynonym_sampling(self, num_samples: int, max_iter: int = 5):
        ...
        for sibling_class_group in tqdm(sibling_class_groups):
...
class CrossOntologyTextSemanticsCorpus:
    ...
    def synonym_sampling_from_mappings(self):
        ...
        for class_mapping in tqdm(self.class_mappings):
    ...
    def __init__(
        ...
        if use_wordnet:
            for onto in [tgt_onto] + self.auxiliary_ontos:
                self.wordnetCorpus = WordNetCorpus(onto.owl_path, annotation_property_iris.source + annotation_property_iris.target + additional_annotation_iris)
